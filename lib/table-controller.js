   ...
   function writeRow () {
      ...
      for (var i=0; i<self.tableArray[self.currentRow].length; i++) {
        ...
        if (maxWidth >= thisWidth) {
          ...
          ctxWrapper.writeTet(self.tableArray[self.currentRow][i], startX, yOffset, self.currentRow===0);
        }
        else {
    ...
    while (it < words.length)
     {
      ...
      ctxWrapper.writeTet(words[it], currX + startingXOffset, ctxWrapper.getRowHeight(rowWrapCartesian, self.currentRow), self.currentRow === 0);
